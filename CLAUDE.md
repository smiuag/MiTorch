...
PENDIENTE (post-Fase 2): migrar el sistema de sonidos auto-detectados (`src/config/soundPatterns.json` + `SoundContext.detectSound`) a la infraestructura de triggers. Reemplaza las 19 reglas hardcoded por una plantilla seeded "Sonidos del MUD" — todos los patrones caben en cajas (los 4 que parecían no encajar son resolubles: drop coma del `[\d,]+` para el patrón de XP (RdL no usa separador de miles, son enteros puros), splits de alternations (`|`) en triggers separados (uno por alternativa, todos disparando el mismo `play_sound`), y el patrón del bloqueo (`\[?El bloqueo '([^']+)' termina\]?`) se queda como 1 ÚNICO trigger con `[[El bloqueo '] [frase] [' termina]]` — los corchetes se asumen siempre presentes (confirmado por el usuario para RdL), y nuestro `[frase]` (`(.+?)` non-greedy) maneja correctamente apóstrofes internas en el nombre del bloqueo, mejor que el `[^']+` original. Quedan ~26 triggers totales en cajas, cero modo experto. Bloqueado por Fase 2 porque al ver triggers de sonido el usuario va a querer poder subir los suyos — mejor empaquetar las dos features juntas. Decisiones pendientes a cerrar antes de implementar: (1) toggle global "Usar sonidos" — mantener como kill switch o eliminar; (2) auto-enable de todos los sonidos al cambiar a blind mode — replicar lógica actual o asumir que la plantilla seeded ya viene con todo enabled; (3) añadir botón preview 🔊 en el selector de sonido del editor de trigger (hoy el modal de Settings lo tiene, al migrar se perdería si no lo trasplantamos). Cambio técnico requerido: mover `triggerEngine.process()` ANTES del early-return del blind mode en `TerminalScreen.processingAndAddLine` (sin esto, líneas blind-filtradas dejan de disparar sonidos — regresión vs comportamiento actual).
